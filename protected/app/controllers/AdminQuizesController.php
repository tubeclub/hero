...
class AdminQuizesController extends BaseController{
	...
	public function listQuizes(){
		...
		$quizesQuery = Quiz::joinStats()->orderBy($sortOptions['sort'], $sortOptions['sortType']);
		if($search){
			$quizesQuery->where('topic', 'like', '%' . $search . '%');
		}
		$quizes = $quizesQuery->paginate(self::$perPage);
		if($search){
	...
	public function createEdit(){
		...
		if(Request::ajax() && Request::isMethod('post')) {
			...
			/*if(!empty(Input::get('test')))
				die(var_dump($quiz));*/
			return View::make('admin/quizes/create')->with(array(
